//! The two themes, the colours of file kinds, and the font.
//!
//! The chrome is Ferret's: quiet, one accent colour, everything else
//! recessive, so the treemap is the loudest thing in the window.
//!
//! Kinds of file get the eight categorical colours of a validated palette,
//! in a fixed order and each tied to one kind for good — a colour never moves
//! to another kind when the view changes. Thirteen kinds do not fit in eight
//! distinguishable colours, so the five that rarely fill a disk share one
...
/// The kinds that get a colour of their own, in palette order.
pub const COLOURED: [Kind; 8] = [
    Kind::Executable,
    Kind::Video,
    Kind::Image,
    Kind::Archive,
...
/// Palette slots 1–8, stepped separately for each surface.
const SERIES_LIGHT: [egui::Color32; 8] = [
    rgb(0x2a, 0x78, 0xd6),
    rgb(0xeb, 0x68, 0x34),
    rgb(0x1b, 0xaf, 0x7a),
    rgb(0xed, 0xa1, 0x00),
...
const SERIES_DARK: [egui::Color32; 8] = [
    rgb(0x39, 0x87, 0xe5),
    rgb(0xd9, 0x59, 0x26),
    rgb(0x19, 0x9e, 0x70),
    rgb(0xc9, 0x85, 0x00),
...
/// Everything folded into "other".
const NEUTRAL_LIGHT: egui::Color32 = rgb(0xb4, 0xb2, 0xa9);
const NEUTRAL_DARK: egui::Color32 = rgb(0x5c, 0x5b, 0x57);

/// The colour of a kind of file.
pub fn kind_colour(theme: Theme, kind: Kind) -> egui::Color32 {
    let series = match theme {
        Theme::Light => &SERIES_LIGHT,
        Theme::Dark => &SERIES_DARK,
    };
    match COLOURED.iter().position(|k| *k == kind) {
        Some(slot) => series[slot],
        None => match theme {
            Theme::Light => NEUTRAL_LIGHT,
            Theme::Dark => NEUTRAL_DARK,
        },
    }
}
...
mod tests {
    ...
    #[test]
    fn a_kind_keeps_its_colour_and_the_rest_share_one() {
        assert_eq!(
            kind_colour(Theme::Dark, Kind::Video),
            kind_colour(Theme::Dark, Kind::Video)
        );
        assert_ne!(
            kind_colour(Theme::Dark, Kind::Video),
            kind_colour(Theme::Dark, Kind::Image)
        );
        assert_eq!(
            kind_colour(Theme::Light, Kind::Code),
            kind_colour(Theme::Light, Kind::Other)
        );
        // Every coloured kind is distinct from the neutral.
        for kind in COLOURED {
            assert_ne!(
                kind_colour(Theme::Light, kind),
                kind_colour(Theme::Light, Kind::Other)
            );
        }
    }
